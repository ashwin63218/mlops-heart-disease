...
Covers Step 5.1 requirements:
  ✅ Preprocessing produces expected shape
  ✅ No nulls in output splits
  ✅ Correct dtypes on output
  ✅ Target is binary (0/1 only)
  ✅ Train/test sizes are consistent with an 80/20 split
  ✅ No data leakage (train and test indices are disjoint)
  ✅ Class balance is preserved across splits (stratification check)
  ✅ Preprocessor is embedded inside the saved Pipeline
...
class TestShape:
    ...
    def test_train_test_split_ratio(self, X_train, X_test):
        ...
        assert abs(len(X_train) - expected_train) <= SPLIT_TOLERANCE, (
            f"Train size {len(X_train)} deviates from expected ~{expected_train}"
        )
        assert abs(len(X_test) - expected_test) <= SPLIT_TOLERANCE, (
...
class TestDtypes:
    def test_X_train_numeric(self, X_train):
        ...
        assert not non_numeric, (
            f"Non-numeric columns in X_train: {non_numeric}"
        )
    ...
    def test_X_test_numeric(self, X_test):
        ...
        assert not non_numeric, (
            f"Non-numeric columns in X_test: {non_numeric}"
        )

    def test_y_train_integer(self, y_train):
        """Target must be integer dtype (0 or 1)."""
        assert pd.api.types.is_integer_dtype(y_train) or pd.api.types.is_float_dtype(y_train), (
            f"y_train dtype unexpected: {y_train.dtype}"
        )

    def test_y_test_integer(self, y_test):
        assert pd.api.types.is_integer_dtype(y_test) or pd.api.types.is_float_dtype(y_test), (
...
class TestTarget:
    ...
    def test_y_train_both_classes_present(self, y_train):
        ...
        assert 0 in unique and 1 in unique, (
            f"y_train is missing a class: found {unique}"
        )

    def test_y_test_both_classes_present(self, y_test):
        unique = set(y_test.unique())
        assert 0 in unique and 1 in unique, (
            f"y_test is missing a class: found {unique}"
        )
    ...
    def test_class_balance_preserved(self, y_train, y_test):
        ...
        assert abs(train_ratio - test_ratio) < 0.05, (
            f"Class ratio drift between splits: "
